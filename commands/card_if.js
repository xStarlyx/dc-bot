...
module.exports = {
    ...
    async execute(client, interaction) {
        ...
        if (bmi > 0) {
            // 如何判斷 大或小
            await interaction.reply(`利奧拉的牌比史考特大${bmi}`); // 如果符合，應該回覆甚麼訊息
        } else if (bmi == 0) {
            // 應該要如何判斷剩下的
            await interaction.reply(`利奧拉的牌和史考特一樣大`); // 如果符合，應該回覆甚麼訊息
        } else {
            // 最後一個應該是甚麼情況
            bmi = -bmi;
            await interaction.reply(`利奧拉的牌比史考特小${bmi}`); // 如果符合，應該回覆甚麼訊息
        }
    },
};
